$(document).ready(()=> {
    const data=JSON.parse(jsonData); //разбор строки
    drawCards(data); //передача полученного объекта в функцию
...
function drawCards (data) {  //рисуем записи блога
    $('.blog-container').html(''); //опустошение контейнера
    data.forEach((item, i) => {
        let card=$(cardHtml);
        card.find('.blog-cover').css('background-image', 'url("img/backgrounds/'+item.image+'")');
        card.find('.blog-title h2').text(item.title);
        card.find('.blog-text p').text(item.text);
        card.find('.blog-published-date').text(item.date);
        let tags='';
        //обходим теги объекта;
        item.tags.forEach((tag, i) =>{
            //cоздаём элемент списка
            tags+='<li><a href="'+tag+'">'+tag+'</a></li>';
        });
        //добавляем объекты в родительский класс <ul>
